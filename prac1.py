...
def create_file():
    def save_data():
        ...
        try:
            ...
            # Append new record
            records.append([ino, iname, qty, expiry_date])
    ...
    tk.Button(form, text="Save", command=save_data, bg="#0080ff", font=("Arial", 20), width=10, fg="black").grid(row=4, column=1, pady=20)
...
def add_at_end():
    ...
    tk.Button(form, text="Save", command=save_data, bg="#0080ff", fg="black", font=("Arial", 20), width=10).grid(row=4,
                                                                                                                 column=1,
                                                                                                                 pady=20)
...
def read_file():
    try:
        with open('Stock.dat', 'rb') as f:
            records = pickle.load(f)
            output = "INo\tName\tQty\tExpiry Date\n" + "\n".join(f"{r[0]}\t{r[1]}\t{r[2]}\t{r[3]}" for r in records)
...
def display_all_items():
    try:
        ...
        if not records:
            ...
            return
        output = "Item Number\t\tItem Name\t\tQuantity\t\tExpiry Date\n" + "\n".join(
            f"{item[0]}\t\t{item[1]}\t\t{item[2]}\t\t{item[3]}" for item in records
        )
